...
MODEL_PATH="$MERGED_ROOT/$MODEL_KEY/weasel"
[ -d "$MODEL_PATH" ] || { echo "[error] merged model not found: $MODEL_PATH (run: bash scripts/run_merge.sh)" >&2; exit 1; }

echo "[serve] $MODEL_KEY  path=$MODEL_PATH  gpus=$GPUS  TP=$TP  port=$VLLM_PORT  name=$VLLM_SERVED_NAME"
